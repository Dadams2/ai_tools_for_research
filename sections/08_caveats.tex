...
\begin{frame}[fragile]{Reproducibility}
  \begin{columns}[T, onlytextwidth]
    \begin{column}{0.55\textwidth}
      ...
      \begin{itemize}
        \item \textbf{Pin model versions} in your methods: \model{claude-3-5-sonnet-20241022}
...
\begin{frame}{IP and Data Privacy}
  \begin{columns}[T, onlytextwidth]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        \item Commercial tools (GPT-4o, Claude) may use inputs for model improvement (check ToS)
...
\begin{frame}{Peer Review Integrity}
  \begin{columns}[T, onlytextwidth]
    \begin{column}{0.55\textwidth}
      \textbf{Venue policies (as of 2024)}\\[0.4em]
      \begin{tabular}{ll}
        \toprule
        \textbf{Venue} & \textbf{Policy} \\
        \midrule
        NeurIPS & Disclosure required \\
        ICML    & Disclosure required \\
        ICLR    & Disclosure required \\
        ACL     & Prohibited for reviews \\
        Nature  & Disclosure required; no AI authorship \\
        Science & Prohibited in peer review \\
        \bottomrule
      \end{tabular}
    \end{column}
    \hfill
  ...
  \begin{columns}[T, onlytextwidth]
    \begin{column}{0.32\textwidth}
      \begin{block}{Compute \& Climate}
        Training a large LLM emits hundreds of tonnes of CO\textsubscript{2}-equivalent. Inference at scale adds ongoing costs~\parencite{bender2021stochastic}. Prefer smaller models when they suffice.
      \end{block}
    \end{column}
    \hfill
    \begin{column}{0.32\textwidth}
